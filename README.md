...
Bybitの全USDT無期限契約ペアの資金調達率（Funding Rate）を定期的に収集し、指定した閾値に基づいて異常な金利を持つ銘柄を問い合わせできるAPIサーバーです。

## 機能

- **データ収集 (Fetcher)**:
  - Bybitから全銘柄の最新のFunding Rateを非同期で高速に取得します。
...
1. **環境変数の設定**
   ...
   - `EXCLUDE_TICKERS`: API/Fetcherの対象から除外する銘柄シンボル（カンマ区切り）。例: `BTRUSDT,EULUSDT`
...
1.  **fetcher (データ取得レイヤー)**
    *   定期的に外部のBybit APIにアクセスし、仮想通貨の資金調達率（Funding Rate）データを取得します。
...
1.  `fetcher`がBybit APIからFunding Rateデータを取得し、共有ボリュームの`./data/cmma.db`に書き込みます。
...
graph TD
    ...
    Fetcher -- "Fetch Funding Rate Data" --> Bybit
